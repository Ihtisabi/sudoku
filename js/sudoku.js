...
const isColsafe = (grid,col,value) => { //cek col
...
const isRowsafe = (grid,row,value) => { //cek raw
...
const isBoxsafe = (grid,box_row,box_col,value) => { //cek box
...
const isSafe = (grid, row, col, value) => { //cek total
    return isColsafe(grid, col, value) && isRowsafe(grid, row, value) && isBoxsafe(grid,row-
    row%3,col-col%3,value) && value !==CONSTANT.UNASSIGNED;
}

const findUnassignedPos = (grid,pos) => {   //cari yang kosong
    for (let row = 0; row<CONSTANT.GRID_SIZE; row++) {
        for (let col = 0; col<CONSTANT.GRID_SIZE; col++) {
